...
export default class Sketch {
	...
	resize() {
		...
		if (this.mouseMaterial) {
			this.mouseMaterial.uniforms.uResolution.value.set(this.width, this.height);
		}
	}
	...
	addObjects() {		
		...
		this.texture = textureLoader.load('./src/routes/68-instanced-lines/instanced-lines-4.png', (texture) => {
	...
	setUpSettings() {
		...
		textureFolder.addBinding(this.settings, 'textureScale', {
			label: 'Scale',
			min: 0.5,
			max: 2.0,
			step: 0.1
		...
		if (this.material) {
			this.material.uniforms.uTextureScale = { value: this.settings.textureScale };
			this.material.uniforms.uBlurStrength = { value: this.settings.blurStrength };
		}
		
		mouseFolder.addBinding(this.settings, 'trailDecay', {
			label: 'Trail Decay',
			min: 0.8,
			max: 0.999,
			step: 0.001
		...
		mouseFolder.addBinding(this.settings, 'trailStrength', {
		...
		mouseFolder.addBinding(this.settings, 'maxIntensity', {
			label: 'Max Intensity',
			min: 0.2,
			max: 1.0,
			step: 0.05
		...
		effectFolder.addBinding(this.settings, 'maxDisplacement', {
			...
			this.material.needsUpdate = true;
				...
				effectFolder.addBinding(this.settings, 'blurStrength', {
			label: 'Blur Strength',
			min: 1.0,
			max: 20.0,
			step: 0.5
	...
	clearTrail() {
		...
		const clearMaterial = new THREE.MeshBasicMaterial({ color: 0x000000, transparent: true, opacity: 0 });
		const clearMesh = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), clearMaterial);
	...
	render() {
		...
		if (mouseDelta > 0.001) {
			...
			this.mouseMaterial.uniforms.uTrailTexture.value = this.renderTarget.texture;
			
			// Update mouse uniforms
			this.mouseMaterial.uniforms.uMousePosition.value.copy(this.mouse);
			this.mouseMaterial.uniforms.uPreviousMousePosition.value.copy(this.previousMouse);
