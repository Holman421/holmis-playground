...
export class MSDFText {
	...
	async loadAssets() {
		...
		this.charMap = new Map(fontData.chars.map((c: MsdfChar) => [String.fromCharCode(c.id), c]));
	}
	...
	public async updateParameters(p: Partial<{ textScale: number; planePadding: number; lineSpacing: number; edgeCenter: number; edgeWidth: number; showCircles: number; circleRadius: number; circleFeather: number; glyphPad: number; glyphPadY: number; }>) {
		...
		if (p.planePadding !== undefined) this.uniforms.planePadding.value = p.planePadding;
		if (p.lineSpacing !== undefined) { this.uniforms.lineSpacing.value = p.lineSpacing; relayout = true; }
		if (p.edgeCenter !== undefined) { this.uniforms.edgeCenter.value = p.edgeCenter; shaderOnly = true; }
		if (p.edgeWidth !== undefined) { this.uniforms.edgeWidth.value = p.edgeWidth; shaderOnly = true; }
	...
	private get spaceAdvance(): number { // derive from metrics (id 32) else fallback
	...
	private computeLineWidth(line: string, scale: number) {
		return Array.from(line).reduce((w, ch) => {
			if (ch === ' ') return w + this.spaceAdvance; const info = this.charMap.get(ch); return info ? w + info.xadvance * scale : w; }, 0);
	}

	private processLine(line: string, scale: number, offset: number) {
		const chars: ProcessedLineChar[] = []; let x = offset; const SPACE = this.spaceAdvance;
		for (const ch of line) {
			if (ch === ' ') { chars.push({ char: ' ', xPosition: x, width: SPACE, uvX1: 0, uvY1: 0, uvX2: 0, uvY2: 0 }); x += SPACE; continue; }
			const c = this.charMap.get(ch); if (!c) continue;
			const w = c.width * scale;
			chars.push({ char: ch, xPosition: x, width: w, uvX1: c.x / this.atlasWidth, uvY1: 1 - (c.y + c.height) / this.atlasHeight, uvX2: (c.x + c.width) / this.atlasWidth, uvY2: 1 - c.y / this.atlasHeight });
			x += c.xadvance * scale;
		}
	...
	private computeLayout() {
		...
		const processed = lines.map((l, i) => this.processLine(l, scale, (this.maxLineWidth - widths[i]) / 2));
		this.lineCharacterData = processed.map((p) => p.characters);
		this.lineActualWidths = processed.map((p) => p.actualWidth);
		const avgCharHeight = this.fontData.chars.reduce((s: number, c: MsdfChar) => s + c.height, 0) / this.fontData.chars.length;
	...
	public async createMesh(forceGeometry = false) {
		...
		if (!this.mesh || forceGeometry) {
			...
			Object.assign(mat, { transparent: true, side: THREE.DoubleSide, depthWrite: false, positionNode: positionLocal });

			this.dispose();
			this.mesh = new THREE.Mesh(geom, mat);
		} else {
			// Optionally adjust geometry scale to approximate new bounds without full recreation
			const g = this.mesh.geometry as THREE.PlaneGeometry; const p = g.parameters;
			this.mesh.scale.set(this.totalWidth / p.width, this.totalHeight / p.height, 1);
		}
		this.rebuildShader();
	}
	...
	private rebuildShader() {
		...
		mat.colorNode = this.buildFontShader(this.lineCharacterData, this.lineActualWidths, this.maxLineWidth)();
		mat.needsUpdate = true;
	}

	/** Helper to process a single line of text into character data */
	private buildFontShader(lineCharacterData: ProcessedLineChar[][], actualWidths: number[], maxActualLineWidth: number) {
		return Fn((): any => {
			const texCoord = varying(uv());
			const lines = this.options.text.split('\n');
			const l1 = lineCharacterData[0] || []; const l2 = lineCharacterData[1] || [];
			varying(positionLocal);
			const paddedUV = texCoord.sub(0.5).mul(this.uniforms.planePadding).add(0.5);
			const inPad = step(float(0.0), paddedUV.x).mul(step(paddedUV.x, float(1.0))).mul(step(float(0.0), paddedUV.y)).mul(step(paddedUV.y, float(1.0)));
			...
			const w1 = float(actualWidths[0] || 0.0), w2 = float(actualWidths[1] || 0.0), wMax = float(maxActualLineWidth || 1.0);
			...
			const pick = (chars: any[], i: number): any => i >= Math.min(chars.length, 64)
				? { uvX1: float(0), uvY1: float(0), uvX2: float(0), uvY2: float(0), localU: float(0), centerX: float(0), width: float(1.0) }
				: (() => {
					const c = chars[i];
					const xs = float(c.xPosition), w = float(c.width), xe = xs.add(w);
			...
			const p1 = pick(l1, 0), p2 = pick(l2, 0);
			const uvX1 = mix(p2.uvX1, p1.uvX1, isLine1), uvY1 = mix(p2.uvY1, p1.uvY1, isLine1), uvX2 = mix(p2.uvX2, p1.uvX2, isLine1), uvY2 = mix(p2.uvY2, p1.uvY2, isLine1), localU = mix(p2.localU, p1.localU, isLine1);
			...
			const charUV = vec2(uvX1.add(localU.mul(uvX2.sub(uvX1))), uvY1.add(yLocal.mul(uvY2.sub(uvY1))));
			const hasUV = step(float(0.001), uvX2.sub(uvX1)).mul(step(float(0.001), uvY2.sub(uvY1)));
			...
			const circleAccum = (chars: any[], i: number, lineY: any): any => i >= Math.min(chars.length, 64)
				...
				: (() => {
					const c = chars[i];
					const xs = float(c.xPosition); const w = float(c.width); const cx = xs.add(w.mul(0.5));
					const isGlyph = step(float(0.001), float(c.uvX2).sub(float(c.uvX1))).mul(step(float(0.001), float(c.uvY2).sub(float(c.uvY1))));
					const cy = lineY.add(charH.mul(0.5));
					const dx = pixelX.sub(cx); const dy = pixelYGlobal.sub(cy);
					const dist = dx.mul(dx).add(dy.mul(dy)).sqrt();
					const inner = radius.mul(float(1.0).sub(feather));
					const contrib = float(1.0).sub(smoothstep(inner, radius, dist)).mul(isGlyph);
