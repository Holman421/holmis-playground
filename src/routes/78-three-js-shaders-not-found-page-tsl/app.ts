...
interface SketchUniforms {
	...
	clickPullRadius: UniformNode<number>;
	clickPullSoftness: UniformNode<number>;
	hoverRadius: UniformNode<number>;
...
export default class Sketch {
	...
	targetSceneHalfWidth: number;
	desktopHoverRadius: number;
	mobileHoverRadius: number;
	desktopBloomStrength: number;
	mobileBloomStrength: number;
	desktopBloomRadius: number;
	mobileBloomRadius: number;
	desktopClickPullRadius: number;
	mobileClickPullRadius: number;
	desktopClickPullSoftness: number;
	mobileClickPullSoftness: number;
	...
	constructor(options: SketchOptions) {
		...
		this.camera.position.set(0.0, 0.0, this.baseCameraDistance);
		...
		this.targetSceneHalfWidth = 3.15;
		this.desktopHoverRadius = 0.1;
		this.mobileHoverRadius = this.desktopHoverRadius * 3;
		this.desktopBloomStrength = 0.3;
		this.mobileBloomStrength = 0.15;
		this.desktopBloomRadius = 0.75;
		this.mobileBloomRadius = 0.2;
		this.desktopClickPullRadius = 0.75;
		this.mobileClickPullRadius = 1.35;
		this.desktopClickPullSoftness = 0.15;
		this.mobileClickPullSoftness = 0.3;

		const isMobileOnInit = window.matchMedia('(max-width: 768px)').matches;
		const particleDensityFactor = isMobileOnInit ? 0.25 : 1.0;

		this.size = Math.max(1, Math.round(256 * Math.sqrt(particleDensityFactor)));
		this.circleCount = this.size ** 2;
		this.streamCount = Math.max(1, Math.round(75000 * particleDensityFactor));
		...
		this.uniforms = {
			...
			clickPullRadius: uniform(this.desktopClickPullRadius),
			clickPullSoftness: uniform(this.desktopClickPullSoftness),
			hoverRadius: uniform(this.desktopHoverRadius),
			...
			bloomStrength: uniform(this.desktopBloomStrength),
			bloomRadius: uniform(this.desktopBloomRadius),
	...
	private createComputeShader() {
		const computeFn = Fn(() => {
			...
			const uClickPullRadius = this.uniforms.clickPullRadius;
			const uClickPullSoftness = this.uniforms.clickPullSoftness;
			...
			const uHoverRadius = this.uniforms.hoverRadius;
			...
			If(instanceIndex.lessThan(this.circleCount), () => {
				newPos.xy.addAssign(
					curlVec.mul(uNoiseStrength).div(uCircleNoiseDivisor)
				);
			}).Else(() => {
				newPos.xy.addAssign(
					curlVec.mul(uNoiseStrength).div(uStreamNoiseDivisor)
				);
			});
			...
			const hoverSoftness = uHoverRadius.mul(0.5);
			const repelDist = length(newPos.xy.sub(mouse)).sub(uHoverRadius);
			const repelInteraction = smoothstep(hoverSoftness, 0.0, repelDist);

			// Click mode: use ~3x larger magnetic radius.
			const pullDist = length(newPos.xy.sub(mouse)).sub(uClickPullRadius);
			const pullInteraction = smoothstep(uClickPullSoftness, 0.0, pullDist);

			// Pseudo-random per-particle mask that gradually includes more particles
			// as click is held, eventually covering all particles.
			const pullSeed = fract(
				sin(info.x.mul(127.1).add(info.y.mul(311.7))).mul(43758.5453)
			);
			const pullMask = smoothstep(
				pullSeed.sub(0.05),
				pullSeed.add(0.05),
				uClickPullProgress
			);
	...
	addObjects() {
		...
		this.material.colorNode = Fn(() => {
			...
			If(bufferIndex.lessThan(this.circleCount), () => {
				...
				const finalCircleColor = animatedColor
					.mul(colorBrightness)
					.mul(progressFade);
				finalColor.assign(
					vec4(finalCircleColor, circleAlpha.mul(progressFade))
				);
			}).Else(() => {
				const simTime = this.uniforms.simTime;
				...
				finalColor.assign(
					vec4(finalStreamColor, animatedAlpha.mul(edgeFade).mul(fadeOut))
				);
			});
	...
	private calculateResponsiveCameraDistance() {
		const halfFovTan = Math.tan(
			THREE.MathUtils.degToRad(this.camera.fov * 0.5)
		);
		const horizontalPadding = 1.08;
		const safeAspect = Math.max(this.camera.aspect, 0.001);
		const requiredDistanceForWidth =
			(this.targetSceneHalfWidth * horizontalPadding) /
			(safeAspect * halfFovTan);

		return Math.max(this.baseCameraDistance, requiredDistanceForWidth);
	}

	private updateResponsiveLayout() {
		this.camera.position.z = this.calculateResponsiveCameraDistance();
	}

	private updateViewportTuning() {
		const isMobileViewport = window.matchMedia('(max-width: 768px)').matches;

		this.uniforms.hoverRadius.value = isMobileViewport
			? this.mobileHoverRadius
			: this.desktopHoverRadius;
		this.uniforms.bloomStrength.value = isMobileViewport
			? this.mobileBloomStrength
			: this.desktopBloomStrength;
		this.uniforms.bloomRadius.value = isMobileViewport
			? this.mobileBloomRadius
			: this.desktopBloomRadius;
		this.uniforms.clickPullRadius.value = isMobileViewport
			? this.mobileClickPullRadius
			: this.desktopClickPullRadius;
		this.uniforms.clickPullSoftness.value = isMobileViewport
			? this.mobileClickPullSoftness
			: this.desktopClickPullSoftness;
	}
	...
	setupButtonHover() {
		...
		const animateUniformTargets = (
			targets: Partial<Record<HoverUniformKey, number>>
		) => {
			for (const [uniformKey, value] of Object.entries(targets) as [
				HoverUniformKey,
				number
			][]) {
	...
	destroy() {
		...
		if (this.hoverButton) {
			...
			if (this.onButtonMouseEnter) {
				this.hoverButton.removeEventListener(
					'mouseenter',
					this.onButtonMouseEnter
				);
			}
