<script lang="ts">
	...
	$effect(() => {
		...
		// Graves
		const graveDeometry = new THREE.BoxGeometry(0.6, 0.8, 0.2);
		...
		for (let i = 0; i < 30; i++) {
			...
			const grave = new THREE.Mesh(graveDeometry, graveMaterial);
		...
		const floor = new THREE.Mesh(
			new THREE.PlaneGeometry(27, 27, 100, 100),
			new THREE.MeshStandardMaterial({
				// color: '#777777',
				transparent: true,
		...
		const tick = () => {
			...
			// Update door light
			doorLight.intensity = Math.sin(elapsedTime * 2) * 2 + 2;
