	...
	sync(callback) {
		...
		this._members.forEach((packingInfo, text) => {
			if (packingInfo.dirty || text._needsSync) {
				...
				(syncPromises || (syncPromises = [])).push(
					new Promise((resolve) => {
						if (text._needsSync) {
							text.sync(resolve);
						} else {
			...
			Promise.all(syncPromises).then(() => {
				...
				let memberIndexes =
					(batchedAttributes[memberIndexAttrName] &&
						batchedAttributes[memberIndexAttrName].array) ||
					new Uint16Array(0);
				let batchedGlyphIndexes =
					(batchedAttributes[glyphIndexAttrName] && batchedAttributes[glyphIndexAttrName].array) ||
					new Float32Array(0);
				let batchedGlyphBounds =
					(batchedAttributes[glyphBoundsAttrName] &&
						batchedAttributes[glyphBoundsAttrName].array) ||
					new Float32Array(0);
				...
				this._members.forEach((packingInfo, { textRenderInfo }) => {
					if (textRenderInfo) {
						...
						batchedGlyphIndexes.set(
							textRenderInfo.glyphAtlasIndices,
							glyphIndex,
							glyphIndex + glyphCount
						);
						batchedGlyphBounds.set(
							textRenderInfo.glyphBounds,
							glyphIndex * 4,
							(glyphIndex + glyphCount) * 4
						);
 ...
 */
export function uniformToVarying(
	{ vertexShader, fragmentShader },
	uniformName,
	varyingName = uniformName
) {
